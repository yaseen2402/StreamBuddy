      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className={`p-3 rounded-lg border-2 transition-all ${
            mode === 'local' ? 'border-primary-600 bg-primary-50' : 'border-gray-200 hover:border-primary-300'
          } ${isActive ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
        >
          <Monitor className={`w-5 h-5 mx-auto mb-1 ${mode === 'local' ? 'text-primary-600' : 'text-gray-600'}`} />
          <div className="text-sm font-semibold text-gray-900">Local</div>
        </button>
